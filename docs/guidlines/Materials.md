Materials
> **Agent application directive — premium UI:** Use materials to establish depth and separate control layers from content, never as ornamental blur. Select regular/stronger material for text-heavy or busy contexts, use clear material only where the background stays legible, and provide opaque/high-contrast fallbacks for reduced transparency. Limit layers and shadows so hierarchy remains obvious.
...
An illustration of a field of view in visionOS with a window in the center. The window has an opaque background that obstructs its surroundings.

An X in a circle to indicate incorrect usage

An illustration of a field of view in visionOS with a window in the center. The window has a translucent material background that allows its surroundings to pass through.
...
An illustration of a Share button with a translucent background material and a symbol. The symbol uses the default vibrant label color and has very high contrast against the background material.
label

An illustration of a Share button with a translucent background material and a symbol. The symbol uses the secondary vibrant label color and has high contrast against the background material.
secondaryLabel

An illustration of a Share button with a translucent background material and a symbol. The symbol uses the tertiary vibrant label color and has muted contrast against the background material.
tertiaryLabel
...
An illustration of a modal view in watchOS with an example title, descriptive text, and a single action button. The modal completely covers the screen with a transparent material, and uses a thinner material for the button along with vibrant label text.
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit material-specification **Output Format**.

- **Select:** Retrieve the component layer, background complexity, text density, theme, and transparency/contrast preference before applying a material.
- **Write/compress:** Keep approved surface roles, opacity/blur ranges, and fallbacks in stable memory; use short notes for exceptions caused by a specific background.
- **Isolate/output:** Assess material separately from content colour. Output the layer role, material recipe, legibility fallback, and test states rather than arbitrary blur/shadow values.
