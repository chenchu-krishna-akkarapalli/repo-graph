Dark Mode
> **Agent application directive — premium UI:** Honour the system appearance and use separate adaptive base/elevated surfaces rather than colour inversion. Validate every foreground, icon, image, border, and status state in light/dark, increased contrast, and reduced transparency; preserve depth and legibility when transparency is removed.
...
A sketch of concentric circles with half-filled areas, suggesting the presence of light and dark. The image is overlaid with rectangular and circular grid lines and is tinted yellow to subtly reflect the yellow in the original six-color Apple logo.
...
A screenshot of the Stocks app on iPhone in its standard dark-only appearance, showing the Apple Inc. stock in detail. The view includes a summary of the current stock price along with a graph of its performance over the past year.
...
An illustration of a square with a light background and four color swatches representing system colors in the light appearance.
System colors in the light appearance

An illustration of a square with a dark background and four color swatches representing system colors in the dark appearance.
...
An illustration of a black droplet icon against a light background.
Icon in the light appearance with no border

An illustration of a black droplet icon against a dark background. The icon has a white border to distinguish it from the similar surrounding color.
...
An illustration of two people sitting at a restaurant table done in a simple, abstract style. The illustration has a light background and its details are clearly visible.
Illustration on a light background

An illustration of two people sitting at a restaurant table done in a simple, abstract style. The illustration has a dark background, and the darker portions of the image are hard to distinguish from the background.
On a dark background, the same illustration has poor contrast and many details are lost

An illustration of two people sitting at a restaurant table done in a simple, abstract style. The illustration has a dark background, and its color values are adjusted to be clearly visible in contrast to the background.
...
An illustration of a button in the light appearance with dark primary label text.
Primary label in the light appearance

An illustration of a button in the dark appearance with light secondary label text.
...
A diagram that shows a stack of 4 terms on top of a black background. The term at the top shows the most contrast with the background and the term at the bottom shows the least.
Base

A diagram that shows a stack of 4 terms on top of a nearly black background. The term at the top shows the most contrast with the background and the term at the bottom shows the least.
Elevated

A diagram that shows a stack of 4 terms on top of a white background. The term at the top shows the most contrast with the background and the term at the bottom shows the least.
Light
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit theme-validation **Output Format**.

- **Select:** Retrieve the component’s semantic tokens plus light, dark, increased-contrast, and reduced-transparency behaviour before styling it.
- **Write/compress:** Preserve only tested theme mappings and exceptions in stable memory; capture failed contrast/transparency cases as concise open notes.
- **Isolate/output:** Treat theme validation as a dedicated pass. Output the adaptive token mapping and test matrix rather than a hard-coded dark-mode override.
