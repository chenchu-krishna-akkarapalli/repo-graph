Windows
> **Agent application directive — premium UI:** Use windows for UI-centric multitasking and let system frames, resizing, focus, safe areas, and glass establish context. Preserve a clear primary task at all sizes, avoid overloading toolbars/ornaments, keep keyboard focus predictable, and use a volume only when spatial depth materially improves the experience.
...
Full screen
Windowed
A screenshot of the Notes app in full screen on iPad, with an open document titled Nature Walks. The app interface fills the entire screen, with no visible border to the window.
...
An illustration of a stack of three windows, as follows: An inactive window in the background, an app’s main window in the middle, and a key window appearing above the other two windows.
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit window-plan **Output Format**.

- **Select:** Retrieve the window type, platform, primary task, resize behaviour, toolbar/ornament requirements, and content depth before changing window composition.
- **Write/compress:** Keep stable window sizing, focus, and restoration rules in project memory; record only active placement or resize observations in the scratchpad.
- **Isolate/output:** Separate window-management concerns from inner component styling. Output window role, adaptive layout rule, focus/keyboard behaviour, and platform-specific fallback.
