Inclusion
> **Agent application directive — premium UI:** Use plain, respectful language; avoid stereotypes, unnecessary gender or culture assumptions, and imagery that narrows who belongs. Design for localisation from the start: allow text expansion, RTL layouts, locale-aware formats, and culturally appropriate colour/copy; never treat a disability as inability to use the product.
...
A sketch of two people, suggesting inclusion. The image is overlaid with rectangular and circular grid lines and is tinted yellow to subtly reflect the yellow in the original six-color Apple logo.
...
A solid silhouette of a person from the shoulders up, within a circle.
person.crop.circle

Solid silhouettes of three people, with the left silhouette in the foreground and the other two in the background, all from the shoulders up.
person.3.fill

A solid silhouette of a person standing with an arm raised high on the left side of the image.
figure.wave
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit inclusive-review **Output Format**.

- **Select:** Retrieve the target audience, locale, language, content scenario, and any known accessibility needs before creating copy or representative imagery.
- **Write/compress:** Preserve inclusive-language decisions, localisation constraints, and validated terminology in long-term memory; keep review findings brief and specific.
- **Isolate/output:** Run inclusion review independently of visual preference. Output biased/exclusionary risk, inclusive replacement, localisation impact, and validation need.
