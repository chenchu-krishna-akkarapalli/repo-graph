Managing accounts
> **Agent application directive — premium UI:** Require an account only for functionality that truly needs identity or persistence. Minimise fields, explain why each is needed, use trusted platform sign-in/autofill where available, and design clear recovery, validation, consent, loading, error, and signed-out states without exposing sensitive data.
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit account-flow **Output Format**.

- **Select:** Retrieve only the requested account action, authentication state, platform, minimum required fields, and privacy/security constraints before designing an account flow.
- **Write/compress:** Store durable consent, credential, and account-state rules securely; retain only non-sensitive workflow progress in short-term notes.
- **Isolate/output:** Isolate account and identity context from unrelated UI tasks. Output a minimal-data flow, validation/error states, and the explanation for each requested field.
