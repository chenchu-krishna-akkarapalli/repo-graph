App icons
> **Agent application directive — premium UI:** For app-icon work, preserve a single, recognisable concept, centred primary content, safe zones, crisp foreground edges, and appropriate light/dark variants. Do not add labels, static highlights, custom shadows, bevels, or ornamental detail that will fight system effects or become illegible at small sizes.
...
A screenshot of the Photos app icon in Icon Composer.
Icon Composer
...
tvOS
visionOS, watchOS
An image of the Settings icon for iOS. The iOS, iPadOS, and macOS icon grid is overlaid on the icon to show how the icon's shape and its elements map to the grid.
...
An image of the Podcasts app icon.
The Podcasts app icon

An image of the Home app icon.
The Home app icon
...
An illustration of two circles centered above a grid. One circle encloses the other. The inner circle has a solid fill. The outer circle is larger than the inner circle, allowing some space between them. The outer circle has no fill and shows just an outline.

An X in a circle to indicate incorrect usage.

An illustration of two circles centered above a grid. One circle encloses the other. The inner circle has a solid fill. The outer circle is larger than the inner circle, has no outline, and has a semi-transparent fill that allows the background grid to show through. Together, the two circles give the impression that the inner circle is resting upon the outer circle.
...
A grid showing the six different appearances of the Photos app icon in iOS. The top row shows the default, clear light, and tinted light icon variants. The bottom row shows the dark, clear dark, and tinted dark variants.
...
A diagram of the Settings icon in tvOS with a white dotted line inside the outer border, which indicates the safe zone.
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit asset-specification **Output Format**.

- **Select:** Retrieve the action meaning, rendering size, surrounding background, platform, and existing icon asset before proposing or changing an app icon.
- **Write/compress:** Save approved icon intent, safe-zone constraints, and appearance variants; do not retain irrelevant asset-production detail in active context.
- **Isolate/output:** Keep app-icon work separate from interface-icon decisions. Output required asset variants, safe-area checks, and accessibility naming needs.
