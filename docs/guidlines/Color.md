Color
> **Agent application directive — premium UI:** Use semantic colour roles and appearance-aware variants, not arbitrary hex values. Apply colour to communicate hierarchy, selection, and status only with a text/icon/shape companion; verify contrast in light, dark, and increased-contrast modes. Keep navigation neutral above visually rich content so controls stay legible.
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit colour-token **Output Format**.

- **Select:** Retrieve the target component, semantic colour role, current appearance mode, and contrast requirement. Do not load or invent a whole palette for a one-component change.
- **Write/compress:** Keep approved semantic tokens and light/dark/high-contrast variants in long-term project memory; note only changed mappings in the scratchpad.
- **Isolate/output:** Evaluate colour independently from layout and motion. Output token substitutions and a contrast/state verification list; never use colour alone as a signal.
