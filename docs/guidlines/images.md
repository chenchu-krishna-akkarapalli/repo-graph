Images
> **Agent application directive — premium UI:** Use images only when they improve comprehension, identity, or task context. Supply correct scale/appearance variants, preserve the focal subject during responsive cropping, provide meaningful alt text for informative imagery, and ensure text or controls over an image remain legible through material, scrim, or placement—not text shadow alone.
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit image-specification **Output Format**.

- **Select:** Retrieve the image purpose, placement, crop/safe-zone requirement, appearance mode, and meaningful alt-text requirement before modifying an image.
- **Write/compress:** Keep approved assets, focal points, and responsive variants as durable facts; store only current crop or optimisation decisions in short-term notes.
- **Isolate/output:** Separate content imagery from decorative effects. Output source/variant, responsive crop rule, contrast treatment, and alt-text decision.
