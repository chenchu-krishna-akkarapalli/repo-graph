...
> **Agent application directive — premium UI:** Add immersion only when it strengthens the primary task. Maintain a stable frame of reference, prioritise legibility and comfort over spectacle, avoid peripheral/high-contrast distraction, and retain a clear non-immersive fallback. Do not introduce spatial motion, scale, or depth without a user-facing purpose and comfort check.
In visionOS, you can design apps and games that extend beyond windows and volumes, immersing people in your content.
A sketch that suggests Apple Vision Pro. The image is overlaid with rectangular and circular grid lines and is tinted yellow to subtly reflect the yellow in the original six-color Apple logo.
...
A screenshot of a window in the Shared Space.
...
A screenshot of an app running in a Full Space using the mixed immersion style in visionOS.
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit experience-plan **Output Format**.

- **Select:** Retrieve the user’s primary task, spatial surface, field-of-view constraints, and interaction distance before adding immersive content or motion.
- **Write/compress:** Keep stable comfort/safety constraints and approved spatial anchors; reduce exploratory alternatives to a short decision note.
- **Isolate/output:** Isolate spatial/immersive work from standard window UI. Output one focused experience plan with grounding, motion-comfort, and fallback considerations.
