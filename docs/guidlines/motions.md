Motion
> **Agent application directive — premium UI:** Animate only to clarify causality, feedback, hierarchy, or state; keep transitions brief, precise, interruptible, and consistent with the triggering gesture. Avoid gratuitous repeated motion, peripheral movement, sustained oscillation, and animations that block action. Always provide a reduced-motion equivalent that preserves the information.
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit motion-specification **Output Format**.

- **Select:** Retrieve the triggering action, intended feedback, frequency, user control, duration, and reduced-motion preference before adding animation.
- **Write/compress:** Store the approved motion token and purpose; summarize interrupted or rejected motion ideas instead of carrying animation detail across turns.
- **Isolate/output:** Review motion as a separate behaviour pass. Output trigger, visible feedback, duration/easing, cancellation path, and reduced-motion fallback.
