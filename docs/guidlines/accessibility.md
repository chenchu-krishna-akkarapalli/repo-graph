Accessibility
> **Agent application directive — premium UI:** Treat this HIG as a release gate, not a final polish pass. For every changed UI, implement semantic controls, visible focus, keyboard operation, labelled inputs, non-colour status cues, 44×44px targets, and light/dark contrast before visual refinement. Test keyboard-only, 200% zoom, reduced motion, and meaningful empty/error/loading states; report any unmet criterion explicitly.
...
- **Seven-piece stack:** Use this file as **Instructions**; pair it with the current **User Input**, selected **Retrieved Facts**, allowed **Tools**, concise **Short-term Notes**, stable project **Long-term Memory**, and an explicit accessible **Output Format**.

- **Select:** Retrieve only the affected control, interaction state, and the relevant requirements for contrast, focus, keyboard operation, labels, motion, or target size.
- **Write/compress:** Store audit findings as short pass/fail notes with the affected selector and remediation; preserve only unresolved risks between turns.
- **Isolate/output:** Keep accessibility review separate from visual polish. Return: semantic issue, user impact, required change, and verification method.
